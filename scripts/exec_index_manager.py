...
def find_task(index: dict, task_id: str) -> dict | None:
    """根据ID查找任务"""
...
def cmd_mark_done(index: dict, task_id: str, deliverable: str = "", coverage: str = "", issues: list = None):
    ...
    # 更新meta统计
    if "meta" in index:
        index["meta"]["completed"] = sum(1 for t in get_all_tasks(index) if t.get("status") == "DONE")
        index["meta"]["in_progress"] = sum(1 for t in get_all_tasks(index) if t.get("status") == "IN_PROGRESS")
        index["meta"]["pending"] = sum(1 for t in get_all_tasks(index) if t.get("status") in ("PENDING", "PENDING_HUMAN"))
        index["meta"]["blocked"] = sum(1 for t in get_all_tasks(index) if t.get("status") == "BLOCKED")

    save_index(index)
    log(f"DONE: {task_id} | {task.get('name', '')}")
    ...
    if next_task:
        ...
        if next_task.get("manual"):
            print(f"   ⏸️  此为人工复核节点，请等待用户确认")
    print()
...
def _find_next_task(index: dict, completed_task_id: str) -> dict | None:
    """找到下一个可执行的任务"""
    tasks = get_all_tasks(index)
    completed_ids = {t["id"] for t in tasks if t.get("status") == "DONE"}
